...
function MaakRouteLayer() {
     var gpx = new ol.layer.Vector({
          source: new ol.source.Vector({
               url: `${PUBLIC_DIR}assets/routes/${RouteName}.gpx`,
...
function MaakWaypointPoint(punt) {
     ...
     var tpointStyle = new ol.style.Style({
          image: new ol.style.Icon({
               anchor: [1, 1],
               src: `${PUBLIC_DIR}assets/herberg.png`
          }
          ),
...
function HaalTrackers() {
     ...
     $.ajax({
          type: "POST",
          url: `${PUBLIC_DIR}api/haaltrackers.php`,
          data: {
               pincode: pincode
          },
...
function SuccessTrackers(trackers) {
     ...
     for (var t in trackers) {
          lon = parseFloat(trackers[t].longitude);
          lat = parseFloat(trackers[t].latitude);
          tps.push(MaakTrackerPoint(lon, lat, -1));
     }
     if (tps.length > 0) {
          if (VGT) {  //  tps.push(MaakTrackerPoint(lon, lat, 1));
...
function MaakFollowLayer() {
     ...
     var iconStyle = new ol.style.Style({
          image: new ol.style.Icon({
               ...
               scale: 0.5,
               opacity: 0.75,
               src: `${PUBLIC_DIR}assets/follow.png`
          }
          )
     }
     );
...
// islaatste = 2 : laatste
{
     var src = `${PUBLIC_DIR}assets/red.png`;
     var scale = 0.3;
     var opacity = 1;
     if (islaatste > 0) {
          scale = 1;
     ...
     if (islaatste > 1) {
          src = `${PUBLIC_DIR}assets/ezel.png`;
          scale = 1;
          opacity = 1;
     }
...
function showHuidigePositie(position) {
     ...
     if (hoek) {
          hoek = hoek;
          mapView.setRotation(hoek); // radialen
     };
     NoError();
...
function saveLocs() {
     ...
     $.ajax({
          type: "POST",
          url: `${PUBLIC_DIR}api/saveloc.php`,
          timeout: 30000,
          data: {
...
function berekenHoek(lat1, lon1, lat2, lon2) {
     let lat = lat2 - lat1;
     let lon = lon2 - lon1;
     let tan = Math.atan(lon / lat);
     return (-tan + Math.PI) % Math.PI
